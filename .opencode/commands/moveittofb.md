---
description: Migrate deployment configuration to Fullbay infrastructure (S3 bucket, DynamoDB, Lambda naming, Cognito, region, DNS zone)
---

Ok. We need to make some changes to the deployment configuration. These changes likely occurring the terraform files, but may need to be made in the lambda function code as well. Specifically, we need to:
