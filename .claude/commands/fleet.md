...
4. Display the launch table:
   ...
   ...
   ```
5. Ask via `AskUserQuestion`: "Proceed with N parallel subagents?" (options: "Yes, dispatch all" / "Show me the spec first" / "Cancel"). On "Show me the spec first", print the spec contents and ask again.

---

## Step 1: Dispatch Subagents in Parallel

In a SINGLE message, send one `Task` tool call per repo using `subagent_type: general-purpose`. Each subagent prompt template (substitute `{repo_path}` and embed the cached spec):
...
2. Run /verify-premise (the premise-verifier subagent) with the spec as input. If verdict is "STOP — premise refuted", stop immediately and report:
...
6. Run /ship-when-clean — autonomous review-and-fix loop with auto-fix for CRITICAL+HIGH. Max 5 iterations.
7. Commit with `git notes`, push, open a PR via `gh pr create --body-file <tmpfile>`.
8. STOP at PR creation. Do NOT trigger /shipit.

Report ONE final line in this format:
  RESULT: repo={repo_path} outcome=<SUCCESS|REFUTED|TEST_FAIL|REVIEW_STUCK|HUMAN_NEEDED|ERROR> branch=<branch-or-->  pr=<#N-or-->  reason=<one-line>

Output only the RESULT line as your final response. Do not include narrative or transcripts. The controller will tabulate.

Per CLAUDE.md Output Token Discipline: keep your internal updates terse; the controller only needs the final RESULT line.
```
...
Reprint only on state change. Use `run_in_background: true` for any controller-level polling Bash.
...
- Per CLAUDE.md Output Token Discipline: dashboard + final report only, no narrative.
- Per CLAUDE.md Heredocs rule: all PR bodies use `--body-file <tmpfile>`, not inline heredoc.
