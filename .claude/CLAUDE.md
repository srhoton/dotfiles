...
- **Atlassian MCP for Jira/Confluence**: all Jira/Confluence operations go through the Atlassian MCP tools when the server is connected (`getJiraIssue`, `searchJiraIssuesUsingJql`, `createJiraIssue`, `addCommentToJiraIssue`, `transitionJiraIssue`, `getConfluencePage`) — never browser automation. If the server isn't connected in a session, say so and ask rather than falling back to the browser. GitHub stays `gh` CLI, not MCP.
